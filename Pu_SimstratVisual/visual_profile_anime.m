...
switch profileFlag
    case 'temp'
        ...
        xlim([Tmin Tmax])
        grid minor
        legText = [obs_datestr; "sim"];
        xlabel(['Temperature [', char(176) ,'C]']); ylabel('Depth [m]')
        legend(legText, 'Location','southeast', 'FontSize', 18)
    case 'Kz'
        yline(-50)
        xlim([-7, 0])
        grid minor
...
fn = sprintf('%s/profile_anime_%s_%s_%s.gif', figDir, profileFlag, fName, string(datetime(mixYY, mixMM, mixDD)));
