% function [] = visual_profile(path2file, profile_year, Tmin, Tmax, dryN, wetN)
...
fig = figure('Name','Profiles');
set(0, 'DefaultAxesFontSize', 18)
grid on
titText = {sprintf('%s day', dryM); ...
    sprintf('%s night', dryM); ...
    sprintf('%s day', wetM);
    sprintf('%s night', wetM)};
for i = 1 : length(selection)
    % temperature plot
    subplot(2, length(selection), i)
    plot(selectedTempArray(i,:), depth, '-r','LineWidth', 3)
    title(titText(i));
    xlabel(['Temp [', char(176) ,'C]']);
    ylabel('Depth [m]');
...
saveas(gcf, sprintf('outputs/figures/profiles_%s_%s.png', fName, string(profile_year)))

save(sprintf('outputs/profiles_%s_%s.mat', fName, string(profile_year)),...
    'selection', 'selectedIndex','selectedlogKzArray', 'selectedTempArray', 'depth');

% observation - model temperature comparison ------------------------------------------------

figure('Name','Temp and CTDs')
...
legend(legText, 'Location','southeast', 'FontSize', 14)

saveas(gcf,sprintf('outputs/figures/tempComp_%s_%s.png', fName, string(profile_year)))
